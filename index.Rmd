...
**Distance (all)**: `r round(sum(yearly_activites$distance), 1)` km  
**Activity time**: `r round(sum(yearly_activites$elapsed_time) / (60*60), 2)` hrs  
**Number of activities**: `r length(yearly_activites$id)`  

**Distance (run)**: `r round(sum(subset(yearly_activites, type == "Run")$distance), 1)` km  
**Distance (ski)**: `r round(sum(subset(yearly_activites, type == "NordicSki")$distance), 1)` km  
**Distance (ride)**: `r round(sum(subset(yearly_activites, type == "Ride")$distance), 1)` km 
...
### Yearly goal
```{r}
current_distance = round(sum(subset(yearly_activites, type == "Run" | type == "NordicSki")$distance), digits = 1)
```
...
```{r}
current_date <- Sys.Date()
cutoff <- Sys.Date() - lubridate::weeks(4)
all_activities <- nrow(subset(df, calendar_date >= cutoff))
run_activities <- nrow(subset(df, calendar_date >= cutoff & type == "Run"))
ride_activities <- nrow(subset(df, calendar_date >= cutoff & type == "Ride"))
ski_activities <- nrow(subset(df, calendar_date >= cutoff & type == "NordicSki"))
```

### Activities during the last four weeks.  **Running:** `r run_activities`,  **Cycling:** `r ride_activities`,  **Nordic ski:** `r ski_activities`.   
```{r}
valueBox(all_activities, icon = "fa-walking")
```
...
hc <- highchart() %>% 
  ...
  hc_series(list(name = "Nordic Ski (km)", data = weekly_dist_wide$NordicSki, color="#DB4437"),
...
hc <- highchart() %>% 
  ...
  hc_series(list(name = "Nordic Ski (km)", data = monthly_dist_wide$NordicSki, color="#DB4437"),
...
Last activity {data-icon="fa-chart-area"}
=======================================================================
Row {data-height=600}
-----------------------------------------------------------------------
### Lastest activity
...
Performance {data-icon="fa-chart-line"}
=======================================================================
### Performance
```{r}
