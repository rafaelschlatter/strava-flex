...
### Last activity Nordic-ski
```{r}
ski_latest_id <- get_latest_activity(yearly_activites, "NordicSki")
ski_stream <- get_streams(stoken, id = ski_latest_id)
ski_stream_clean <- remove_latlng(ski_stream)
ski_stream_df <- stream_to_df(ski_stream_clean)
...
hc <- highchart() %>% 
  ...
  hc_add_series(yAxis=1, ski_stream_df, "area", hcaes(x=distance, y=altitude),
                name = "Altitude", color = "gray") %>%
  hc_add_series(yAxis=0, ski_stream_df, "line", hcaes(x=distance, y=heartrate), name = "Heartrate") %>%
  hc_tooltip(sort = TRUE, table = TRUE) %>%
  hc_add_theme(hc_theme_google())
hc
```
