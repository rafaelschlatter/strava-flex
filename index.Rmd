...
```{r}
monthly_dist <- group_by(yearly_activites, calendar_year, month, type) %>% summarize(dist = sum(distance))
monthly_dist_wide <- spread(monthly_dist, type, dist)
monthly_dist_wide$month <- factor(monthly_dist_wide$month, levels = month.name)
monthly_dist_wide <- monthly_dist_wide[order(c(monthly_dist_wide$calendar_year, monthly_dist_wide$month)), ]
