...
stream_to_df <- function(stream) {
  for (i in seq(length(stream))) {
    ...
    # because cannot deal with this type yet
    if (column_name == "latlng") {
      next
    }
    if (i == 1) {
...
run_stream_df <- stream_to_df(run_stream)
...
hc <- highchart() %>% 
  ...
  hc_add_series(yAxis=1, run_stream_df, "area", hcaes(x=distance, y=column_name),
...
ski_stream_df <- stream_to_df(ski_stream)
...
hc <- highchart() %>% 
  ...
  hc_add_series(yAxis=1, ski_stream_df, "area", hcaes(x=distance, y=column_name),
                name = "Altitude", color = "gray") %>%
