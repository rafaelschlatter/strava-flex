...
stream <- get_streams(stoken, id = 2261424151)
stream_df <- stream_to_df(stream)

hc <- highchart() %>% 
  hc_chart(type = "line") %>% 
  hc_add_series(stream_df, "line", hcaes(distance, heartrate), name = "Heartrate") %>%
  hc_add_series(stream_df, "line", hcaes(distance, cadence), name = "cadence") %>%
